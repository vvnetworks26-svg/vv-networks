...
const EngagementCard = memo(function EngagementCard({ model, index, onSelect }: EngagementCardProps) {
  ...
  return (
    <motion.article
      className={`relative bg-white rounded-3xl border flex flex-col h-full overflow-hidden ${
        model.highlight
        ...
        shouldReduce
          ...
          : {
              ...
              boxShadow: model.highlight
                ? "0 28px 60px -15px rgba(79,70,229,0.2)"
                : "0 20px 50px -12px rgba(9,13,26,0.13)",
            }
      }
    >
      {/* Highlight top bar */}
      {model.highlight && (
        <div
          className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-brand-blue via-brand-indigo to-brand-violet"
          aria-hidden="true"
        />
      )}
      {model.highlight && (
      ...
      <div className={`p-6 bg-gradient-to-br ${model.gradient} border-b border-brand-slate-200/60`}>
        ...
        <p className="text-xs text-brand-slate-500 leading-relaxed mt-1.5">{model.tagline}</p>

        <div className="mt-5 flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5 text-brand-slate-400" aria-hidden="true" />
            <span className="text-[11px] font-semibold text-brand-slate-500">{model.deliveryTime}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Users className="w-3.5 h-3.5 text-brand-slate-400" aria-hidden="true" />
            <span className="text-[11px] font-semibold text-brand-slate-500">{model.support}</span>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-1.5">
            {model.deliverables.map((item) => (
              <div key={item} className="flex items-start gap-2">
                ...
                <span className="text-xs text-brand-slate-600">{item}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {model.outcomes.map((outcome) => (
              <div key={outcome} className="flex items-start gap-2">
                ...
                <span className="text-xs text-brand-slate-600">{outcome}</span>
              </div>
            ))}
          </div>
        </div>
