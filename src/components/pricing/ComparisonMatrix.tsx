...
const columnHeaders = ["LeadFlow", "Growth Website", "Custom Software", "Enterprise"];
const columnKeys: Array<keyof typeof matrixRows[0]> = ["leadflow", "growthWeb", "customSoft", "enterprise"];
...
const categories = ["Process", "AI", "Support"];
  ...
  return (
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px]" role="table" aria-label="Engagement model comparison">
          {/* Header */}
          <thead>
            <tr className="border-b border-brand-slate-200">
              <th scope="col" className="text-left px-6 py-4 text-[10px] font-mono uppercase tracking-wider text-brand-slate-400 font-bold w-[38%]">
                What's included
              </th>
              {columnHeaders.map((col, i) => (
                <th key={col} scope="col" className={`px-4 py-4 text-center text-xs font-bold text-brand-navy w-[15.5%] ${i === 2 ? "text-brand-indigo" : ""}`}>
                  {col}
                </th>
              ))}
            </tr>
            ...
            {categories.map((category) => {
              ...
              return (
                <React.Fragment key={category}>
                  {/* Category sub-header */}
                  <tr className="bg-brand-slate-50/60">
                  ...
                  {rows.map((row, rowIdx) => {
                    ...
                    return (
                      <motion.tr
                        key={row.label}
                        className={`border-t border-brand-slate-100 cursor-default transition-colors ${
                          isHovered ? "bg-brand-blue/3" : "hover:bg-brand-slate-50/50"
                        }`}
                        ...
                        viewport={{ once: true }}
                        transition={{ duration: shouldReduce ? 0.1 : 0.3, delay: shouldReduce ? 0 : rowIdx * 0.04 }}
                      >
      ...
      <div className="px-6 py-4 bg-brand-slate-50 border-t border-brand-slate-200 flex flex-wrap gap-5">
        {[
          { icon: <Check className="w-3 h-3 text-emerald-600" />, bg: "bg-emerald-50", label: "Included" },
          { icon: <CircleDot className="w-3 h-3 text-amber-500" />, bg: "bg-amber-50", label: "Partially included" },
          { icon: <Minus className="w-3.5 h-3.5 text-brand-slate-300" />, bg: "bg-white", label: "Not included" },
        ].map(({ icon, bg, label }) => (
          <div key={label} className="flex items-center gap-2">
            <div className={`w-5 h-5 rounded-full ${bg} flex items-center justify-center border border-brand-slate-200`} aria-hidden="true">
              {icon}
            </div>
