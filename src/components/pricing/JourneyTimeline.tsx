...
import { journeySteps } from "./pricingData";
...
const JourneyTimeline = memo(function JourneyTimeline() {
  ...
  return (
    <div className="space-y-3" role="list">
      ...
      <div className="hidden lg:flex items-center justify-between mb-8 relative px-6">
        <div className="absolute left-6 right-6 top-5 h-0.5 bg-brand-slate-200 z-0">
          <motion.div
            className="h-full bg-gradient-to-r from-brand-blue via-brand-violet to-teal-500 origin-left"
            initial={{ scaleX: 0 }}
            whileInView={{ scaleX: 1 }}
            viewport={{ once: true }}
            transition={{ duration: shouldReduce ? 0.05 : 1.6, ease: "easeOut", delay: shouldReduce ? 0 : 0.2 }}
          />
        </div>
        {journeySteps.map((step, i) => (
          <motion.button
            ...
            className={`relative z-10 flex flex-col items-center gap-2 group focus-visible:outline-2 focus-visible:outline-brand-blue rounded-xl p-1`}
            aria-pressed={expandedStep === i}
            aria-label={`${step.title} — ${step.duration}`}
            initial={{ opacity: 0, y: shouldReduce ? 0 : 10 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: shouldReduce ? 0.1 : 0.35, delay: shouldReduce ? 0 : i * 0.07 }}
          >
            <motion.div
              className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-black font-mono text-white shadow-sm ${step.color} ${
                expandedStep === i ? "ring-2 ring-offset-2 ring-current" : ""
              }`}
            ...
            <span className={`text-[10px] font-mono font-bold uppercase tracking-wider transition-colors ${
              expandedStep === i ? "text-brand-navy" : "text-brand-slate-400 group-hover:text-brand-navy"
            }`}>
              {step.title}
            </span>
          </motion.button>
                ...
                >
                  <div className="px-5 pb-5 pt-4 border-t border-brand-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="space-y-2.5">
                      <span className="text-[10px] font-mono uppercase tracking-wider text-brand-slate-400 font-bold flex items-center gap-1.5">
